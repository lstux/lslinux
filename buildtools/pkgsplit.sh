...
while getopts d:l:scC0123456789vh opt do; case "${opt}" in
  d) case "${OPTARG}" in
       "ev") DEVPKG=true;;
       "oc") DOCPKG=true;;
       *)    usage;;
...
if ${BINSTRIP}; then
  msg "Stripping binaires"
  find "${INSTALLDIR}" -type f -executable | while read f; do
    case "$(file "${f}")" in
      *"not stripped") nfo "  stripping ${f}"; strip --strip-unneeded "${f}";;
      *" stripped")    dbg "  already stripped ${f}";;
      *)               dbg "  executable unstrippable ${f}";;
    esac
  done
fi
